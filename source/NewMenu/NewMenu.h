...
typedef int (__cdecl *STRINGCOMPAREFN)(CTreeName * elem1, CTreeName * elem2); 


class CSortableMenuArray : public CArray<CTreeName,CTreeName>
{
public:
	CString GetTagName(int imageIndex);
	void Sort(STRINGCOMPAREFN pfnCompare = Compare){
	CTreeName * prgstr = GetData();
	qsort(prgstr,GetSize(),sizeof(CTreeName),(GENERICCOMPAREFN)pfnCompare);
	
	
	};
protected:
	static int __cdecl Compare(CTreeName * pstr1,CTreeName * pstr2){
			return pstr1->MenuName.Compare(pstr2->MenuName);
...
public:
  ...
  BOOL AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID, CString strDirectory_Name);
  ...
  CNewMenu* AppendODPopupMenu(LPCTSTR lpstrText, int nIconNormal = -1 );
