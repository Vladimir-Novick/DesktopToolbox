...
{
	return RegOpenKeyEx(hKey,         // handle to open key
	...
	return RegOpenKey(hKey,         // handle to open key
       lpSubKey,  // subkey name
       phkResult    // handle to open key
  );
...
{
  strcpy(UserKeyPatch,_T("Software\\NVR Soft\\MyDesktopToolbox"));
//		char szName[60];
//		unsigned long ls=60;
...
void CRegisterSave::AutoRun(BOOL flag)
{
		...
		const char s_ValueName[] = _T("DesktopToolbox");
...
void CRegisterSave::UserKeyStore(const char *key, long *value,unsigned long LenRead,Storage StoreFlag)
{
    char BufText[255];
		 BufText[0]=0;
	switch(StoreFlag)
...
void CRegisterSave::UserKeyStore(const char *key, char *value,unsigned long LenRead,Storage StoreFlag)
{
		HKEY hkeyRun;
...
void CRegisterSave::UserKeyStore(char *key, long &value,Storage StoreFlag)
{
    char BufText[255];
		 BufText[0]=0;
	switch(StoreFlag)
...
void CRegisterSave::UserKeyStore(char *key, int &value,Storage StoreFlag)
{
    char BufText[255];
		 BufText[0]=0;
	switch(StoreFlag)
