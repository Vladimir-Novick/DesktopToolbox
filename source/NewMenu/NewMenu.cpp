...
HBITMAP LoadColorBitmap(LPCTSTR lpszResourceName, HMODULE hInst, int* pNumcol)
{
  if(hInst==0)
  {
    hInst = _Module.GetModuleInstance(); //  originl -> =AfxFindResourceHandle(lpszResourceName, RT_BITMAP);
  }
...
BOOL CNewMenuIcons::LoadToolBar(LPCTSTR lpszResourceName, HMODULE hInst)
{
  ...
  if(hInst==0)
  {
    hInst = _Module.GetModuleInstance(); // AfxFindResourceHandle(lpszResourceName, RT_TOOLBAR);
  }
...
void CNewMenu::DrawMenuTitle(LPDRAWITEMSTRUCT lpDIS, BOOL bIsMenuBar)
{
  ...
  if(pMenuData->m_nTitleFlags&MFT_LINE)
  {
    if(pMenuData->m_nTitleFlags&MFT_SIDE_TITLE)
    {
      CRect rect2(rect.left+20,rect.top+5,rect.left+22,rect.bottom-1);
...
//      pDC->Draw3dRect(rect2,GetSysColor(COLOR_3DSHADOW),GetSysColor(COLOR_3DHILIGHT));
    ...
    {
      CRect rect2(rect.left+2,rect.bottom-1,rect.right+5,rect.bottom-1);
...
BOOL CNewMenu::AppendODMenu(LPCTSTR lpstrText, UINT nFlags, UINT nID,
                            CString strDirecoryName)
{

char FindFileList[255];

UINT nIDNew = nID;

    strcpy(FindFileList, strDirecoryName.GetBuffer(1)); 
    strcat(FindFileList, "*.*"); 
...
BOOL fFinished = FALSE; 
  ...
  HANDLE hFind;
  hFind = FindFirstFile(FindFileList, &FindFileData);
...
if ((FindFileData.cFileName[0] != '.') )
{
	...
	char szNewPath[255];
    lstrcpy(szNewPath, strDirecoryName.GetBuffer(1)); 
...
char newName[255];
lstrcpy(newName,lpstrText);
lstrcat(newName,FindFileData.cFileName);


if (   (0 != (FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))){
	  CNewMenu* pSubMenu = NewFolder->AppendODPopupMenu(_T("Insert Top Folder"),IDB_FOLDER);
} else {


	AppendODMenu(newName, nFlags, nIDNew,
                            IDB_RUN_SELECTED);
}
...
{
  HINSTANCE hInst = _Module.GetModuleInstance(); // AfxFindResourceHandle(MAKEINTRESOURCE(nResourceId),RT_BITMAP);
...
CNewMenu* CNewMenu::AppendODPopupMenu(LPCTSTR lpstrText,int nIconNormal )
{
  ...
  if(AppendODMenu(lpstrText,MF_POPUP,HMenuToUInt(pSubMenu->m_hMenu), nIconNormal))
  {
