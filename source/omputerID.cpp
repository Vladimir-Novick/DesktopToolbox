// omputerID.cpp: implementation of the ComputerID class.
//
//////////////////////////////////////////////////////////////////////
...
void ComputerID::DecryptEncrypt(char * inputString, int operation, int random_key)
{
   ...
   EncryptionKey= (EncryptionKeys[random_key]);

	int totalLen = strlen(inputString);
    char  outString[255];// (' ', totalLen); //create an empty string
...
base64_encode((unsigned char *)m_sz256String,strlen(m_sz256String),(unsigned char *)temp2,&pLen);
  int iu;

  for (iu=0;temp2[iu] != 0; iu++){
    if (temp2[iu]=='-'){ temp2[iu] = '9';}
    if (temp2[iu]=='#'){ temp2[iu] = 'Z';}
    if (temp2[iu]=='N'){ temp2[iu] = '0';}

  }
	strcpy(m_sz256String,temp2);
		...
		value[0] = 0;
		DWORD LenRead = 255;
	    RegOpenKeyEx(HKEY_LOCAL_MACHINE,temp1,0,KEY_READ,&hkeyRun);
 ...
 sprintf(m_sz256String,"%d",kp);
  ...
  AddCheckSum(m_sz256String);
   ...
   char *key3 = "{GCx"; //"~MHz";
		...
		RegQueryValueEx(hkeyRun,temp2,NULL,&dwType,(unsigned char *)szString,&LenRead);
		DWORD wTime;
		memcpy(&wTime,szString,4);
		if (wTime+ 100 > 1000 ) {
		  wTime = (wTime+100) /1000;
		  wTime = wTime*1000 - 100;
		} else {
		  wTime = wTime /100;
		  wTime = wTime*100;
		}
		memcpy(szString,&wTime,4);
...
void ComputerID::GetUserID(char *strID, char *srtKey)
{
long keyNum;
keyNum = 0;
int i;
	for (i = 0; i < strlen(strID);i++){
		keyNum = keyNum+i%6 + strID[i];
	}
	char szString[255];
  sprintf(szString,"%d",keyNum);
  AddCheckSum(szString);
  ...
  strcpy(szString,strID);
   AddCheckSum(szString);
   DecryptEncrypt(szString,ENCRYPT,1);
  strcat(srtKey,szString);
